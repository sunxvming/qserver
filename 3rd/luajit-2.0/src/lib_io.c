...
// fancy
} else if (tvisudata(tv) && udataV(tv)->metatable.gcptr32==basemt_it(G(L), LJ_TSTR).gcptr32) {
MSize len = udataV(tv)->len;
status = status && (fwrite((char*)uddata(udataV(tv)), 1, len, fp) == len);
    } else if (tvisint(tv)) {
...
// fancy
#if LJ_TARGET_POSIX
	int fd = fileno(io_stdfile(L, id));
	if (id==GCROOT_IO_INPUT)
		dup2(fd, fileno(stdin));
	else if (id==GCROOT_IO_OUTPUT)
...
LJLIB_CF(io_input)
{
  return io_std_getset(L, GCROOT_IO_INPUT, "rb"); // fancy
}

LJLIB_CF(io_output)
{
  return io_std_getset(L, GCROOT_IO_OUTPUT, "ab"); // fancy
}
