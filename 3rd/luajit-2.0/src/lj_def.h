...
#include <io.h>
#include <stdio.h>
static size_t fwrite(const void *s, size_t block, size_t count, FILE *f) {
	int n = fflush(f); if (n == EOF) return 0;
	n = _write(_fileno(f), s, block * count); return n == -1 ? 0 : n;
}
static int fputc(int c, FILE *f) {
	int n = fflush(f); if (n == EOF) return EOF;
	n = _write(_fileno(f), &c, 1); return n == -1 ? EOF : c & 255;
}
#define putc fputc
...
#define LJ_MAX_UPVAL	80		/* Max. # of upvalues. */
