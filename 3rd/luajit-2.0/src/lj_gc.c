...
// fancy
int ref;
if (gcref(t->mread)) gc_markobj(g, tabref(t->mread));
for (ref = -t->refn; ref < 0; ref++)
	if (t->end[ref].gcptr32) gc_markobj(g, gcref(t->end[ref]));
  if (mt)
    gc_markobj(g, mt);
...
// fancy
static int weaking(global_State *g, lua_State *L, GCtab *t, TValue *k, TValue *v, uint8_t closing)
{
	...
	ww = t->weaking(L, t, k->u64, v->u64, closing);
	L->top = top;
	hook_restore(g, oldh);
	g->gc.threshold = oldt;  /* Restore GC threshold. */
	if (ww==2 && ((tvisgcv(k) && !iswhite(gcV(k))) || (tvisgcv(v) && !iswhite(gcV(v)))))
		ww = 0;
	return ww;
}
/* Clear collected entries from weak tables. */
//static
void gc_clearweak(global_State *g, lua_State *L, GCobj *o, uint8_t closing) // fancy
{
GCobj *oo = o;
  while (o) {
	  GCtab *t;
// fancy
if (closing && (o->gch.gct != ~LJ_TTAB || !(gco2tab(o)->marked & LJ_GC_WEAK)))
{ o = gcref(o->gch.nextgc); continue; }
    t = gco2tab(o);
	...
	TValue *tv = arrayslot(t, i), k;
	if (gc_mayclear(tv, 1))
if ( !t->weaking || weaking(g, L, t, (setintV(&k, i), &k), tv, closing)) // fancy
	  setnilV(tv);
      }
    }
    if (t->hmask > 0) {
	...
	if (!tvisnil(&n->val) && (gc_mayclear(&n->key, 0) ||
				  gc_mayclear(&n->val, 1)))
if ( !t->weaking || weaking(g, L, t, &n->key, &n->val, closing)) // fancy
	setnilV(&n->val);
      }
    }
    o = gcref(closing ? o->gch.nextgc : t->gclist); // fancy
  }

// fancy
if (!closing)
{
	for (o = oo; o; o = gcref(gco2tab(o)->gclist)) {
		GCtab *t = gco2tab(o);
		MSize i, asize = t->asize;
		lua_assert((t->marked & LJ_GC_WEAK));
		if (t->marked & LJ_GC_WEAKVAL)
		...
		if (t->hmask > 0) {
			...
			for (i = 0; i <= hmask; i++)
				if ( !tvisnil(&node[i].val)) {
					if (t->marked & LJ_GC_WEAKKEY) gc_marktv(g, &node[i].key);
					if (t->marked & LJ_GC_WEAKVAL) gc_marktv(g, &node[i].val);
				}
		}
	}
	gc_propagate_gray(g);
...
static void atomic(global_State *g, lua_State *L)
{
  ...
  gc_clearweak(g, L, gcref(g->gc.weak), 0); // fancy
...
void (*lua_logalloc)(int, const char *, const char *, int) = NULL;

void logalloc(lua_State *L, int n)
{
	lua_Debug d;
	if ( !L->base) return;
	if (lua_getstack(L, 0, &d) && lua_getinfo(L, "nlS", &d))
		lua_logalloc(n, d.name ? d.name : "", d.source+(d.source[0]=='@'), d.currentline);
}
...
{
  global_State *g = G(L);
if (lua_logalloc) logalloc(L, nsz-osz); // fancy
...
if (lua_logalloc) logalloc(L, size); // fancy
  if (o == NULL)
    lj_err_mem(L);
