...
cTValue *lj_meta_tget(lua_State *L, cTValue *o, cTValue *k)
{
  ...
  for (loop = 0; loop < LJ_MAX_IDXCHAIN; loop++) {
    ...
    if (LJ_LIKELY(tvistab(o))) {
      ...
      if (!tvisnil(tv)) return tv;
// fancy
if ((tr = tabref(t->mread)) && !tvisnil(tv = lj_tab_get(L, tr, k))) return tv;
if (tr && (tr = tabref(tr->mread)) && !tvisnil(tv = lj_tab_get(L, tr, k))) return tv;
	  if (!(mo = lj_meta_fast(L, tabref(t->metatable), MM_index))) return tv;
