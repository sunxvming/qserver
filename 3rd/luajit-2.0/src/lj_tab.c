...
//fancy
#define newtab(L, asize, hbits) (newtabbody((L), (asize), (hbits), 2, 0))

/* Create a new table. Note: the slots are not initialized (yet). */
static GCtab *newtabbody(lua_State *L, uint32_t asize, uint32_t hbits, uint16_t intn, uint16_t refn) // fancy
{
  GCtab *t;
int len;
lua_assert(intn >= 2 && refn <= intn);
  /* First try to colocate the array part. */
  if (LJ_MAX_COLOSIZE != 0 && asize > 0 && asize <= LJ_MAX_COLOSIZE) {
    lua_assert((sizeof(GCtab) & 7) == 0);
    t = (GCtab *)lj_mem_newgco(L, len = sizetabcolo(asize)+(intn<<2)+4); // fancy
    t->gct = ~LJ_TTAB;
    t->nomm = (uint8_t)~0;
    t->colo = (int8_t)asize;
    ...
    t = lj_mem_newgco(L, len = sizeof(GCtab)+(intn<<2)+4); // fancy
    t->gct = ~LJ_TTAB;
    t->nomm = (uint8_t)~0;
    t->colo = 0;
...
//fancy
t->weaking = NULL;
setgcrefnull(t->mread);
t->intn = intn;
t->refn = refn;
t->end = (GCRef*)((char*)t+len);
t->end[-1-intn].gcptr32 = (int)t;
*(long long*)(t->end-intn) = 0;
  if (hbits)
    newhpart(L, t, hbits);
lua_calltabcb(L, t);
  return t;
...
// fancy
GCtab *lj_tab_newb(lua_State *L, uint32_t asize, uint32_t hbits, uint16_t len, uint16_t refn)
{
GCtab *t = newtabbody(L, asize, hbits, len, refn);
int ref;
clearapart(t);
if (t->hmask > 0) clearhpart(t);
...
lua_assert(kt->intn==2 && kt->refn==0 && *(long long*)(kt->end-2)==0); // fancy
...
// fancy
lj_mem_free(g, t, (char*)t->end-(char*)t);
