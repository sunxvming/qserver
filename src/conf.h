...
convtable(lua_State *L) {
	int i;size_t sizearray;int sizehash;
	struct context *ctx = (struct context *)lua_touserdata(L,2);
	struct table *tbl = ctx->tbl;

	tbl->L = ctx->L;

	sizearray = lua_objlen(L, 1);
	if (sizearray) {
		...
		tbl->sizearray = (int)sizearray;
	}
	sizehash = countsize(L, (int)sizearray);
	if (sizehash) {
...
convert_stringmap(struct context *ctx, struct table *tbl) {
	...
	lua_pushvalue(L, 1);
	s = (struct state *)lua_newuserdata(L, sizeof(*s));
	s->dirty = 0;
	s->ref = 0;
	s->root = tbl;
	lua_replace(L, 1);
...
lookup_key(struct table *tbl, uint32_t keyhash, int key, int keytype, const char *str, size_t sz) {
	...
	if (tbl->sizehash == 0)
		return NULL;
	n = &tbl->hash[keyhash % tbl->sizehash];
...
lindexconf(lua_State *L) {
	...
	n = lookup_key(tbl, keyhash, key, keytype, str, sz);
	if (n) {
...
lboxconf(lua_State *L) {
	...
	c = (struct ctrl *)lua_newuserdata(L, sizeof(*c));
	c->root = tbl;
	c->update = NULL;
...
luaopen_conf_core(lua_State *L) {
	...
	//luaL_checkversion(L);
	luaL_register (L, "__luaconfcore", l);
